...
def train(input_file, vocab_size, model_name, model_type, character_coverage, output_path):
    """
    ...
    cmd = input_argument % (input_file, output_path + model_name, vocab_size, model_type, character_coverage)
...
def run():
    ...
    en_vocab_size = config.src_vocab_size
    en_model_name = 'eng'
    en_model_type = 'bpe'
    en_character_coverage = 1
    train(en_input, en_vocab_size, en_model_name, en_model_type, en_character_coverage, output_path)

    ch_input = '../data/corpus.ch'
    ch_vocab_size = config.tgt_vocab_size
    ch_model_name = 'chn'
    ch_model_type = 'bpe'
    ch_character_coverage = 0.9995
    train(ch_input, ch_vocab_size, ch_model_name, ch_model_type, ch_character_coverage, output_path)
